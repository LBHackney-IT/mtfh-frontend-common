...
exports.endCautionaryAlert =
  exports.addCautionaryAlert =
  exports.getPropertyCautionaryAlerts =
  exports.usePropertyCautionaryAlert =
  exports.useCautionaryAlertByAlertId =
  exports.useCautionaryAlert =
    void 0;
...
const useCautionaryAlert = (id, options) => {
  return (0, http_1.useAxiosSWR)(
    id && `${config_1.config.cautionaryApiUrlV1}/cautionary-alerts/persons/${id}`,
    options,
  );
};
...
const useCautionaryAlertByAlertId = (alertId, options) => {
  return (0, http_1.useAxiosSWR)(
    alertId && `${config_1.config.cautionaryApiUrlV1}/cautionary-alerts/alert/${alertId}`,
    options,
  );
};
...
const usePropertyCautionaryAlert = (propertyRef, options) => {
  return (0, http_1.useAxiosSWR)(
    propertyRef &&
      `${config_1.config.cautionaryApiUrlV1}/cautionary-alerts/properties-new/${propertyRef}`,
    options,
  );
};
...
const getPropertyCautionaryAlerts = async (assetId) => {
  return new Promise((resolve, reject) => {
    http_1.axiosInstance
      .get(
        `${config_1.config.cautionaryApiUrlV1}/cautionary-alerts/properties-new/${assetId}`,
        {
          headers: {
...
const addCautionaryAlert = async (data) => {
  const { data: alert } = await http_1.axiosInstance.post(
    `${config_1.config.cautionaryApiUrlV1}/cautionary-alerts/`,
    data,
  );
  return {
    ...alert,
...
const endCautionaryAlert = async (alertId, data) => {
  await http_1.axiosInstance.patch(
    `${config_1.config.cautionaryApiUrlV1}/cautionary-alerts/alerts/${alertId}/end-alert`,
    data,
  );
};
