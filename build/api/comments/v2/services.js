...
const useComments = (id, { pageSize = 5, ...options } = {}) => {
    return (0, http_1.useAxiosSWRInfinite)((page, previous) => {
        ...
        return `${config_1.config.notesApiUrlV2}/notes?${(0, query_string_1.stringify)(params)}`;
    }, options);
};
exports.useComments = useComments;
const addComment = async (data) => {
    const auth = auth_1.$auth.getValue();
    const { data: comment } = await http_1.axiosInstance.post(`${config_1.config.notesApiUrlV2}/notes`, {
        ...data,
