...
const addProcess = async (data, processName) => {
  const { data: process } = await http_1.axiosInstance.post(
    `${config_1.config.processApiUrlV1}/process/${processName}`,
    data,
  );
  return process;
};
...
const useProcess = ({ id, processName }, options) => {
  return (0, http_1.useAxiosSWR)(
    `${config_1.config.processApiUrlV1}/process/${processName}/${id}`,
    options,
  );
};
...
const useProcesses = (id, { pageSize = 5, ...options } = {}) => {
  return (0, http_1.useAxiosSWRInfinite)((page, previous) => {
    ...
    return `${config_1.config.processApiUrlV1}/process?${(0, query_string_1.stringify)(
      params,
    )}`;
  }, options);
};
...
const editProcess = async ({ id, processName, processTrigger, ...data }) => {
  const response = await http_1.axiosInstance.patch(
    `${config_1.config.processApiUrlV1}/process/${processName}/${id}/${processTrigger}`,
    data,
  );
