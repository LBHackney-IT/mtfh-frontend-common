...
const searchAddress = async (postCode, structure) => http_1.axiosInstance
    .get(`${config_1.config.addressApiUrlV1}/addresses?postcode=${postCode}${structure ? `&Structure=${structure}` : ""}`, {
    headers: {
...
const getAddressViaUprn = async (UPRN, isParentUPRN, page, pageSize) => {
    return new Promise((resolve, reject) => {
        http_1.axiosInstance
            .get(`${config_1.config.addressApiUrlV1}/addresses?${isParentUPRN ? `parentUprn` : `uprn`}=${UPRN}${page ? `&page=${page}` : ``}${pageSize ? `&pageSize=${pageSize}` : ``}`, {
            headers: {
                "skip-x-correlation-id": true,
            },
        })
...
const useAddressLookup = (postCode, options = {}) => {
    return (0, http_1.useAxiosSWR)(postCode ? `${config_1.config.addressApiUrlV1}/addresses?postcode=${postCode}` : null, {
        ...options,
        timeout: 5000,
        headers: {
            ...options.headers,
