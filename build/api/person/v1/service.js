...
const usePerson = (id, options) => {
    return (0, http_1.useAxiosSWR)(id && `${config_1.config.personApiUrlV1}/persons/${id}`, options);
};
exports.usePerson = usePerson;
const addPerson = async (data) => {
    const { data: person } = await http_1.axiosInstance.post(`${config_1.config.personApiUrlV1}/persons`, data);
    (0, http_1.mutate)(`${config_1.config.personApiUrlV1}/persons/${person.id}`, person, false);
    return person;
};
exports.addPerson = addPerson;
const editPerson = async ({ id, ...data }) => {
    const response = await http_1.axiosInstance.patch(`${config_1.config.personApiUrlV1}/persons/${id}`, data);
