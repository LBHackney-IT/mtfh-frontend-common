...
exports.updatePropertyPatch = exports.createAsset = exports.patchAssetAddress = exports.patchAsset = exports.getParentAssets = exports.getChildAssets = exports.useChildAssets = exports.useAsset = exports.getAsset = void 0;
...
const useAsset = (id, options) => {
    return (0, http_1.useAxiosSWR)(id && `${config_1.config.assetApiUrlV1}/assets/${id}`, options);
};
exports.useAsset = useAsset;
const useChildAssets = (id, options, pageSize = 1000) => {
    const response = (0, http_1.useAxiosSWR)(id &&
        `${config_1.config.assetSearchApiUrlV1}/search/assetrelationships?searchText=${id}&pageSize=${pageSize}`, options);
    return response;
};
exports.useChildAssets = useChildAssets;
const getChildAssets = async (id, pageSize = 1000) => {
    return http_1.axiosInstance.get(`${config_1.config.assetSearchApiUrlV1}/search/assetrelationships?searchText=${id}&pageSize=${pageSize}`);
};
...
const getParentAssets = (parentAssetIds, options) => {
    ...
    if (parentAssetIds) {
        ...
        parents.forEach((p) => {
            ...
            const { data: parentAsset } = (0, http_1.useAxiosSWR)(p && `${config_1.config.assetApiUrlV1}/assets/${p}`, options);
...
const patchAsset = async (id, request, assetVersion) => {
    return http_1.axiosInstance.patch(`${config_1.config.assetApiUrlV1}/assets/${id}`, request, {
        headers: {
...
const createAsset = async (request) => {
    var _a, _b;
    if (((_b = (_a = request.assetLocation) === null || _a === void 0 ? void 0 : _a.parentAssets) === null || _b === void 0 ? void 0 : _b.length) > 1) {
